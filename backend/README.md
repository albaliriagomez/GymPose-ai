...
- Cada ejercicio incluye metadatos para que el frontend sepa cÃ³mo renderizarlo y cÃ³mo medirlo:
  - `name`
  - `sets`
  - `reps`
  - `rest_seconds`
...
- Regla recomendada para frontend:
  - `mode="live"`: ejercicio pensado para flujo con cÃ¡mara / detecciÃ³n
  - `mode="timer"`: ejercicio basado en tiempo
  - `mode="hold"`: sostÃ©n isomÃ©trico
  - si no hay `mode`, tratarlo como ejercicio manual por repeticiones

### Ejercicios soportados actualmente
El catÃ¡logo activo del backend fue reducido al conjunto de ejercicios que sÃ­ se usarÃ¡n en la app.
...
### Nota para frontend
- El frontend no deberÃ­a depender de un catÃ¡logo hardcodeado como fuente principal.
- La fuente de verdad debe ser la respuesta del backend en `days[].exercises[]`.
- Para decidir la experiencia:
  - usar `mode="live"` para flujo con cÃ¡mara
  - usar `mode="timer"` para contador regresivo
  - usar `mode="hold"` para sostener postura
  - usar reps manuales cuando el ejercicio no tenga detecciÃ³n automÃ¡tica
